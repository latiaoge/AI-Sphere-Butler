...
export default function Settings({ wallpaperSettings, setWallpaperSettings, panels = [], resetAllPanels, resetSinglePanel }) {
  ...
  // *** 修改：上传背景文件函数，使用动态 URL 并处理返回路径设置 mediaSrc ***
  const uploadBackgroundFile = async (fileToUpload) => {
    ...
    try {
      ...
      let newMediaSrc = null;
      if (result.frontend_path) {
        newMediaSrc = result.frontend_path;
      } else if (result.file_url) {
        newMediaSrc = result.file_url;
      }
  
      if (newMediaSrc) {
        // 确保以 / 开头，避免路径错误（一般后端已经带 /）
        if (!newMediaSrc.startsWith('/')) {
          newMediaSrc = '/' + newMediaSrc;
        }
        // 不拼接域名，直接用相对路径，加时间戳防缓存
        newMediaSrc += `?t=${Date.now()}`;
