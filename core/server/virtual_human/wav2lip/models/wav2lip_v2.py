...
class Wav2Lip(nn.Module):
    ...
    def inference(self, audio_embedding, face_sequences):
        ...
        for f in self.face_decoder_blocks:
            ...
            batch_size_feat = feats[-1].size(0)
            target_batch_size = min(batch_size_x, batch_size_feat)
            if batch_size_x != target_batch_size:
                x = x[:target_batch_size]
            if batch_size_feat != target_batch_size:
                feats[-1] = feats[-1][:target_batch_size]

            try:
                x = torch.cat((x, feats[-1]), dim=1)
            except Exception as e:
                print("Error cat shapes:")
