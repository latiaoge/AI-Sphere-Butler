...
# --- 高德地图API配置 ---
AMAP_API_KEY = "e37e4d63fc"  # 请替换为你的高德地图API Key
...
def trigger_location() -> bool:
    ...
    """
    try:
        r = requests.post("https://192.168.1.77:5000/trigger-location", timeout=5, verify=False)
        if r.status_code == 200:
            logger.info("触发获取位置成功")
            return True
        logger.error(f"触发获取位置失败: {r.status_code}")
        return False
...
def get_driving_route(origin: str, destination: str) -> Optional[dict]:
    ...
    try:
        ...
        response = requests.get(DIRECTION_URL, params=params, timeout=5)
        data = response.json()
        if data["status"] == "1":
            ...
            return {
                ...
                "steps": [step["instruction"] for step in route.get("steps", [])]  # 导航步骤
            }
        else:
...
def handle_map(text: str, user_location: Optional[str] = None) -> Optional[str]:
    """
    统一处理接口，调用地图请求处理
    """
    return handle_map_request(text, user_location)
...
if __name__ == "__main__":
    ...
    # 模拟用户位置（可改成实际经纬度）
    test_user_location = "116.481488,39.990464"

    for text in test_texts:
        logger.info(f"输入：{text}")
        result = handle_map(text, user_location=test_user_location)
        if result:
            print(result)
        else:
            print("未识别的地图请求")
