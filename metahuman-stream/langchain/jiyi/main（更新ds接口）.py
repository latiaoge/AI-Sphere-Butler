...
from ollama_client import call_tencent  # 替换为腾讯云 API 调用
...
async def generate_response(question: str, records: list, model: str, temperature: float = 0.1, top_p: float = 0.9) -> str:
    ...
    try:
        ...
        # 调用腾讯云 API 生成回复
        response_text = await call_tencent(
        ...
        if not isinstance(response_text, str):
            raise ValueError(f"腾讯云 API 返回值无效，期望是字符串，但得到：{type(response_text)}")
        ...
        logger.info(f"腾讯云 API 回复: {response_text}")
        return response_text
    except IndexError:
        logger.error("未找到相关记录，无法生成回复。")
        return "抱歉，我没有找到相关的记录。"
    except ValueError as ve:
        logger.error(f"参数校验错误: {ve}")
        return "抱歉，输入参数格式有误，请检查后重试。"
    except Exception as e:
        logger.exception(f"调用腾讯云 API 时发生错误: {e}. Question: {question}, Model: {model}")
...
def cache_response(redis_client, cache_key: str, response_text: str, ttl: int = 3600):
    ...
    """
    try:
        if redis_client and response_text not in ["抱歉，腾讯云 API 请求超时，请稍后再试。"]:
