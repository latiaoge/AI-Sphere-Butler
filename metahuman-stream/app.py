...
def llm_response(message):
    ...
    #llm = LLM().init_model('Gemini', model_path= 'qwen1.5-chat',api_key='not empty', proxy_url=http://127.0.0.1:9997/v1)
    ...
    llm = LLM().init_model('Qwen', model_path= 'THUDM/chatglm3-6b', api_key='None', proxy_url='None')
...
async def offer(request):
    ...
    async def on_connectionstatechange():
        print("Connection state is is is %s" % pc.connectionState)
...
async def say5(request):
    params = await request.json()
    print('----say5---'+params['text'])
    
    file9 = open("autosay.log", "w")
    file9.write(params['text'])
    file9.close()
...
async def qwener(request):
    ...
    params = await request.json()
    with open('question.log', 'a') as file:
        # 写入新行或其他需要添加的内容
        file.write(','+params['text'])
    '''
    print(params['text'])
    headers = {'Content-Type': 'application/json'}
    post_data = {"model": "qwen2.5-3bnsfwny","stream": True,"messages": [{"role": "user","content": params['text']}],"max_tokens": 512,"temperature": 0.7}
    response = requests.post('http://192.168.168.7:11434/v1/chat/completions', headers=headers, json=post_data, stream=True)
    ...
    if response.status_code == 200:
      ...
      for line in response.iter_lines():
        if line:
            # 处理接收到的每一行数据
            # 例如，这里可以将接收到的数据转化为字符串
            data_str = line.decode('utf-8')
            print('----------------------')
            data_str = re.sub(r'^data: ', '', data_str)
            if data_str != '[DONE]':
              print(data_str)
              print('======')
              data_str  = json.loads(data_str)
              data_str = data_str['choices'][0]['delta']
              if 'content' in data_str:
                print(data_str['content'])
                with open('finalsay.log', 'a') as file:
                  file.write(data_str['content'])
...
async def say(request):
    ...
    '''
    nerfreals[0].put_msg_txt(params['text'])
...
async def human(request):
    ...
    nerfreals[sessionid].put_msg_txt(params['text'])
    '''
    if params['type']=='echo' and False:
        nerfreals[sessionid].put_msg_txt(params['text'])
    elif params['type']=='chat' or True:
        res=await asyncio.get_event_loop().run_in_executor(None, llm_response(params['text']))                          
...
async def run(push_url):
    ...
    async def on_connectionstatechange():
        ...
        file8.write('')
        file8.close()
        print("Connection state is is %s" % pc.connectionState)
        if pc.connectionState == "failed":
            await pc.close()
            pcs.discard(pc)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tts', type=str, default='VolcanoTTS') #xtts gpt-sovits VolcanoTTS edgetts
    parser.add_argument('--REF_FILE', type=str, default=None)
    parser.add_argument('--REF_TEXT', type=str, default=None)
    parser.add_argument('--TTS_SERVER', type=str, default='https://openspeech.bytedance.com/api/v1/tts') # http://localhost:9000 http://127.0.0.1:9880
    ...
    parser.add_argument('--push_url', type=str, default='http://192.168.1.9:1985/rtc/v1/whip/?app=live&stream=livestream') #rtmp://localhost/live/livestream
    ...
    server = pywsgi.WSGIServer(('0.0.0.0', 8010), app, handler_class=WebSocketHandler)
